...
import { sampleWithRequiredData } from '../discipline.test-samples';
...
describe('Discipline Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.disciplines()[0]).toEqual(expect.objectContaining({ id: 8998 }));
  });

  describe('trackId', () => {
    it('should forward to disciplineService', () => {
      const entity = { id: 8998 };
      jest.spyOn(service, 'getDisciplineIdentifier');
      const id = comp.trackId(entity);
      expect(service.getDisciplineIdentifier).toHaveBeenCalledWith(entity);
