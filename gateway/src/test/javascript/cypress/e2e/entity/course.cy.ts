...
describe('Course e2e test', () => {
  ...
  const courseSample = { courseTitle: 'bowler nor father' };
  ...
  describe('new Course page', () => {
    ...
    it('should create an instance of Course', () => {
      cy.get(`[data-cy="courseTitle"]`).type('discourse luck ski');
      cy.get(`[data-cy="courseTitle"]`).should('have.value', 'discourse luck ski');

      cy.get(`[data-cy="courseDescription"]`).type('chunder aha following');
      cy.get(`[data-cy="courseDescription"]`).should('have.value', 'chunder aha following');

      cy.get(`[data-cy="coursePrice"]`).type('24821');
      cy.get(`[data-cy="coursePrice"]`).should('have.value', '24821');

      cy.get(`[data-cy="courseLevel"]`).select('INTERMEDIATE');
