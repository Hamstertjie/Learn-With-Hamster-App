...
public class LessonGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all lessons").get("/services/service/api/lessons").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new lesson")
                        .post("/services/service/api/lessons")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created lesson").get("/services/service${new_lesson_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created lesson").delete("/services/service${new_lesson_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
