...
public class CourseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all courses").get("/services/service/api/courses").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new course")
                        .post("/services/service/api/courses")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created course").get("/services/service${new_course_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created course").delete("/services/service${new_course_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
