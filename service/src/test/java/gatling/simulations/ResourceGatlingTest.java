...
public class ResourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all resources").get("/services/service/api/resources").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new resource")
                        .post("/services/service/api/resources")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created resource").get("/services/service${new_resource_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created resource").delete("/services/service${new_resource_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
