...
public class ProgramGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all programs").get("/services/service/api/programs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new program")
                        .post("/services/service/api/programs")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created program").get("/services/service${new_program_url}").headers(headersHttpAuthenticated)).pause(10)
                )
                .exec(http("Delete created program").delete("/services/service${new_program_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
