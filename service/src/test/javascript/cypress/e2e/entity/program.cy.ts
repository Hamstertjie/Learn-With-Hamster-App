...
describe('Program e2e test', () => {
  const programPageUrl = '/service/program';
  const programPageUrlPattern = new RegExp('/service/program(\\?.*)?$');
  const username = Cypress.env('E2E_USERNAME') ?? 'user';
  const password = Cypress.env('E2E_PASSWORD') ?? 'user';
  const programSample = { programName: 'so' };
  ...
  it('Programs menu should load Programs page', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem('service/program');
  ...
  describe('Program page', () => {
    describe('create button click', () => {
      ...
      it('should load create Program page', () => {
        ...
        cy.url().should('match', new RegExp('/service/program/new$'));
  ...
  describe('new Program page', () => {
    ...
    it('should create an instance of Program', () => {
      cy.get(`[data-cy="programName"]`).type('pfft');
      cy.get(`[data-cy="programName"]`).should('have.value', 'pfft');

      cy.get(`[data-cy="programDescription"]`).type('ah inspection about');
      cy.get(`[data-cy="programDescription"]`).should('have.value', 'ah inspection about');

      cy.get(`[data-cy="programPrice"]`).type('25313');
      cy.get(`[data-cy="programPrice"]`).should('have.value', '25313');
