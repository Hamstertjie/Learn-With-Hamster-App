...
export class LessonComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['lessonTitle', 'lessonDescription', 'language'];
  ...
  search(query: string): void {
    ...
    if (query && predicate && LessonComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && LessonComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ILesson[] | null): ILesson[] {
    ...
    if (this.links().prev) {
      ...
      if (data) {
        for (const d of data) {
          if (lessonsNew.some(op => op.id === d.id)) {
            lessonsNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.lessonService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
